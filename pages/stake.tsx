...
const Stake: NextPage = () => {
  ...
  const { contract: stackContract, isLoading } = useContract(
    stakingContractAddress,
  )
  ...
  useEffect(() => {
    ...
    const loadClaimableRewards = async () => {
      const stakeInfo = await stackContract?.call('getStakeInfoForToken', [
        0,
        address,
      ])
  ...
  return (
    <>
      <Head>
        <title>Stake Contract</title>
        <meta
          name="description"
          content="thirdweb Deploy - Custom Staking Contract"
        />
